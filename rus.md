...
элементом теневого дерева**. Содержимое ведущего элемента не отображается, 
вместо него отображается содержимое корневого элемента теневого дерева. 
...
необходомости учитывать этот нюанс в коде обновления имени.
...
`content` — это приглашение, которое впускает контент из страницы в служебные 
помещения теневой модели документа.** Эти приглашения доставляются по очереди; 
приглашение получает тот, кто прописан как получатель (с помощью атрибута 
`select`). Контент после получения приглашения отправляется на праздник. Если на 
тот же адрес отправлено ещё одно приглашение, то оно никого не застаёт дома и 
никто больше не придёт на ваш праздник. 
...
<iframe src="http://result.dabblet.com/gist/6741181/c985f12074ea5abf4b9d2dbc8985840fdde63795"></iframe>

## Конец первой части

Это всё основы теневой модели документа, вы успешно ознакомились с первой частью 
нашего курса! С помощью теневой модели документа можно осуществить намного 
